...
//   // 현재 입력란에 타이핑 중인 문자열을 인식하기 위한 것
//   const [text, setText] = useState('');
//   // '저장하기' 버튼을 누르면 기록들을 보관: 배열
//   const [savedList, setSavedList] = useState([]);
//   // 입력란에 변화가 발생할 때마다 실행되어 상태를 갱신
//   const handleChange = (e) => {
//     setText(e.target.value);
//   };
//   // 현재 입력된 내용을 기록 배열에 추가
//   const handelSave = () => {
//     // 문자열이 있는 경우에만 저장하도록 미리 대비하자
//     if (text.trim() !== '') {
//       // 기존 배열(preList)을 가져와서 그 뒤에 text로 새로운 내용 추가
//       setSavedList((preList) => [...preList, text]);
//       // 저장이 완료되면 다음 입력을 위하여 입력란을 청소하자
//       setText('');
//     }
//   };
//   // 현재 입력란의 내용만 지우기
...
//     setSavedList([]);
//   };
//   // for문으로 반복을 사용하여 저장된 기록 목록을 생성한다
//   // <li>
//   const historyElements = [];
//   for (let i = 0; i < savedList.length; i++) {
...
//         value={text}
//         onChange={handleChange}
//         //Enter키를 눌러도 저장되도록 하기
//         onKeyDown={(e) => key === 'Enter' && handlesave()}
//       ></input>
//       {/* 항상 웹 브라우져에는 사용자에게 실시간으로 정확한 정보를 전달하는 것이 좋다  */}
//       <div>
//         <p>실시간 미러링</p>
//         <h3>{text || '입력 대기중...'}</h3>
//       </div>
//       {/* 버튼: 저장하기, 초기화  */}
//       <div>
//         <button onClick={handelSave}>저장하기</button>
